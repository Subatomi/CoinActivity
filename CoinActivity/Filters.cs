...
namespace CoinActivity
{
    ...
    public class Coins
    {
        ...
        public static void CountCoin(Bitmap bmp, ref Label countLabel, ref Label valueLabel)
        {
            ...
            //List<int> coinRadii = new List<int>();
            int width = bmp.Width;
            ...
            using (Graphics g = Graphics.FromImage(bmp)) 
            {
                ...
                for (int i = 0; i < height; i++)
                {
                    for (int j = 0; j < width; j++)
                    {
                        ...
                        {
                            count++;
                            int coinRadius = BFS(bmp, ref visited, j, i, g, pen); 
                            //coinRadii.Add(coinRadius);

                            if (coinRadius >= 1) 
                            {
                                value += Classify(coinRadius, ref count); 
                            }
                ...
                //Debug.WriteLine("Coin Radii: " + string.Join(", ", coinRadii));

                countLabel.Text = "Total Coins: " + count.ToString(); // Show total number of coins
                valueLabel.Text = "Total Value: " + value.ToString("F2");
            }
        }
        ...
        private static int BFS(Bitmap bmp, ref bool[,] visited, int x, int y, Graphics g, Pen pen)
        {
            int height = bmp.Height;
            int width = bmp.Width;
            int count = 0;
            ...
            unsafe
            {
                ...
                for (int i = 0; i < srcHeight; i++, pLoaded += padding)
                {
                    for (int j = 0; j < srcWidth; j++, pLoaded += 3)
                    {
                        ...
                        byte intensity = (byte)((pLoaded[0] + pLoaded[1] + pLoaded[2]) / 3);

                        // Apply threshold
                        byte binaryColor = (intensity < threshold) ? (byte)0 : (byte)255;

                        // Update pixel (all channels set to binary color)
                        pLoaded[0] = pLoaded[1] = pLoaded[2] = binaryColor;
                    }
                }
            }
